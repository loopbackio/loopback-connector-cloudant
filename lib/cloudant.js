...
function findRecursive(mo, query, docs, include, cb) {
  mo.db.find(query, function(err, rst) {
    ...
    if (query.sort) {
      // sort back the ids in "numerical order" if they are numeric
      query.sort.forEach(function(queries) {
        if (queries.hasOwnProperty('_id:number')) {
          var sortType = queries['_id:number'];
          if (Array.isArray(rst.docs)) {
            if (sortType === 'desc')
              rst.docs.sort(function(a, b) {
...
/**
 * Delete all instances for the given model and the model design doc
 * @param {String} model The model name
...
Cloudant.prototype.destroyAll = function destroyAll(model, where, options, cb) {
  ...
  self.all(model, {where: where}, {raw: true}, function(err, docs) {
    ...
    async.each(docs, function(doc, cb2) {
      ...
      if (err) return cb(err);
      if (_.isEmpty(where) || options.ddocDelete) {
        mo.db.list({
          key: '_design/lb-index-ddoc-' + model,
        }, function(err, ddoc) {
          if (err) cb(err);
          mo.db.destroy(ddoc.rows[0].id, ddoc.rows[0].value.rev,
            function(err, result) {
              if (err) return cb(err);
              cb(err, {count: dels});
            });
        });
      } else cb(err, {count: dels});
    });
  });
};
