...
Cloudant.prototype.save = function (model, data, options, cb) {
  ...
  return this.create (model, data, options, cb);
}
...
Cloudant.prototype.all = function all (model, filter, options, cb) {
  var self = this,
      mo = self.selectModel (model),
      query = {
        selector: self.buildSelector (model, filter.where),
...
Cloudant.prototype.updateOrCreate = function (model, data, cb) {
  console.log ('Cloudant.prototype.updateOrCreate %j %j', model, data);
  this.create (model, data, {}, function (err, id, rev) {
    if (err) return cb (err);
    data.id = id;
    cb (null, data);
  });
...
Cloudant.prototype.selectModel = function (model, migrate) {
  var dbName, modelView, dateFields = [],
      db, mo, s = this.settings;
...
Cloudant.prototype.updateIndex = function (mo, modelName, cb) {
  mo.db.index ({
    ...
    "index": {
      ...
      "selector": {
        "loopback__model__name": modelName
      }
    }
  }, function (err, result) {
